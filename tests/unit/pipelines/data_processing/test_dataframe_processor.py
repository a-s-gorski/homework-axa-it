...
from mlstream.pipelines.data_processing.dataframe_processor import DataFrameProcessor
...
def test_binary_threshold_encoding():
    ...
    p = make_processor(target_source="x", binary_rule="threshold", binary_threshold=1.0)
    out = p.process(df)
...
def test_validate_logs_missing_and_extra_columns_and_dtype_warnings(caplog):
    ...
    assert any(
        "Missing columns in DataFrame" in rec.message and "missing_col" in rec.message
        for rec in errors
    )
    ...
    assert any(
        "Extra columns in DataFrame" in rec.message and "extra_col" in rec.message
        for rec in warns
    )

    # Non-categorical one-hot warning
    assert any(
        "is not categorical (will still be one-hot encoded)" in rec.message
        for rec in warns
    )
